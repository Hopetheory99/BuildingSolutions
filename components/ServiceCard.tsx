...
const ServiceCard: React.FC<ServiceCardProps> = ({ icon, title, description, className, ...props }) => {
  return (
    <div 
      className={`group bg-gray-800 p-8 rounded-lg shadow-lg text-center transform transition-all duration-300 hover:scale-105 hover:shadow-xl hover:bg-gray-750 ${className}`}
      {...props}
    >
      ...
      <p className="mt-2 text-gray-400 group-hover:text-gray-300 transition-colors duration-300">{description}</p>
    </div>
  );
};
